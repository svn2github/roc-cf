					...
					<cfquery name="GetBans" datasource="#config.DSN#">
						SELECT * FROM bans WHERE value = '#session.username#' AND expire > UNIX_TIMESTAMP() OR value = '#CGI.REMOTE_ADDR#' AND expire > UNIX_TIMESTAMP() ORDER BY ID DESC LIMIT 1
					</cfquery>
							...
							<ul class="stats" style="position:relative;">
								...
								<td>
									...
									<cfoutput>
									<cfif StructKeyExists(form, "reason") AND reason eq "logout"><h3>Reason: Connection Dropped by Server</h3>
									<cfelseif StructKeyExists(form, "reason")>
									<h3>Error: #reason# </h3>
									...
									<cfif isdefined("error_desc")><h3>Error Notice: #error_desc# </h3></cfif>
									</cfoutput>
